locals {
  master_ips = concat([ for instance in scaleway_instance_ip.master : instance.address ], [scaleway_instance_ip.bootstrap.address])
}

locals {
  worker_ips = [ for instance in scaleway_instance_ip.worker : instance.address]
}
